...
| [PLAN_reliability_tail.md](PLAN_reliability_tail.md) | open; items 1–5 and 7 shipped 2026-08-16, **item 6 is the only one left** | what the 24/7 audit found and the same-day fixes did not take. Shipped: the read cap (streaming, two caps, a legible truncation marker), the undisposed `JsonDocument`, the payload budget's per-call cost (a cheap upper-bound accept plus a single forward pass), explicit Kestrel and request timeouts in `appsettings.json` — with **no** global request-timeout policy, because it would have severed the SSE stream — and the console sink's per-line lock (rendered outside it, one write inside). Item 5 was answered by the operator with a third option neither candidate named: a file per run, **segmented at UTC midnight**, in both the log and the spool. Left: `Mcp.Ui` is built and shipped but wired to nothing. Two things named as still open rather than folded in — nothing prunes old day folders, so total disk is unbounded; and the midnight segment makes this repository's logging differ from the shared rule and from the two sibling repos |
