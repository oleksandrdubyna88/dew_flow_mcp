...
| [PLAN_reliability_tail.md](PLAN_reliability_tail.md) | open; **every item shipped 2026-08-16 on this side**, held open by item 6's consuming half and two named decisions | what the 24/7 audit found and the same-day fixes did not take. Shipped: the read cap (streaming, two caps, a legible truncation marker), the undisposed `JsonDocument`, the payload budget's per-call cost (a cheap upper-bound accept plus a single forward pass), explicit Kestrel and request timeouts in `appsettings.json` — with **no** global request-timeout policy, because it would have severed the SSE stream — and the console sink's per-line lock (rendered outside it, one write inside). Item 5 was answered by the operator with a third option neither candidate named: a file per run, **segmented at UTC midnight**, in both the log and the spool. Item 6 was wired rather than removed: `Mcp.Ui` gained a real `/mcp` page — the human-readable face of the `SurfaceFingerprint` — plus a read side where an unreachable daemon and a server with no tools render as two different messages. Its consuming half (nav entry, router slice, AppHost URL) lives in `dew_flow_rag_qln` and is gated on moving that repo's `external/dew_flow_mcp` submodule pin, which needs a push. Two more things named as open rather than folded in — nothing prunes old day folders, so total disk is unbounded; and the midnight segment makes this repository's logging differ from the shared rule and from the two sibling repos |
